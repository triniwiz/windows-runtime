...
pub(crate) fn ro_resolve_namespace(
    name: &windows::HSTRING,
    ...
    unsafe {
        ...
        let windows_meta_data_dir = match windows_meta_data_dir {
            None => 0 as _,
            Some(windows_meta_data_dir) => windows_meta_data_dir.abi(),
        } as HSTRING;
        let mut package_graph_dirs = match package_graph_dirs {
            None => 0 as _,
            Some(package_graph_dirs) => package_graph_dirs.abi(),
        };
        ...
        let mut meta_data_file_paths = match meta_data_file_paths {
            None => 0 as _,
            Some(meta_data_file_paths) => meta_data_file_paths.abi(),
        };
        ...
        let mut sub_namespaces = match sub_namespaces {
            None => 0 as _,
            Some(sub_namespaces) => sub_namespaces.abi(),
        };
        ...
        core_bindings::RoResolveNamespace(
            name,
            windows_meta_data_dir,
            package_graph_dirs_count,
            package_graph_dirs,
            meta_data_file_paths_count.unwrap_or(0 as _),
            meta_data_file_paths,
            sub_namespaces_count,
            sub_namespaces,
        )
    }
}
...
pub mod rometadataresolution {
    use crate::prelude::*;
    use windows::{Abi, HRESULT, HSTRING};

    pub(crate) fn ro_get_meta_data_file(
        name: &mut windows::HSTRING,
        meta_data_dispenser: Option<*mut c_void>,
        meta_data_file_path: Option<*mut c_void>,
        meta_data_import: Option<*mut *mut IMetaDataImport2>,
        type_def_token: Option<*mut u32>,
    ) -> HRESULT {
        unsafe {
            windows::HRESULT(core_bindings::Rometadataresolution_RoGetMetaDataFile(
                std::mem::transmute(name),
                meta_data_dispenser.unwrap_or(0 as _),
                meta_data_file_path.unwrap_or(0 as _),
                meta_data_import.unwrap_or(0 as _) as _,
                type_def_token.unwrap_or(0 as _),
            ) as u32)
        }
    }
    ...
    pub(crate) fn ro_parse_type_name(
        ...
        unsafe {
            HRESULT(core_bindings::Rometadataresolution_RoParseTypeName(
                type_name,
                parts_count,
                type_name_parts,
            ) as u32)
    ...
    pub(crate) fn ro_locator(
        ...
        unsafe { core_bindings::Rometadataresolution_Ro_Locator(fn_, locator) }
    }
}
...
pub mod imeta_data_import2 {
    ...
    pub(crate) fn get_type_def_props(metadata: &mut IMetaDataImport2, md_type_def: c_uint, sz_type_def: Option<&mut [u16]>, cch_type_def: Option<ULONG>, pch_type_def: Option<*mut ULONG>, pdw_type_def_flags: Option<*mut ULONG>, md_token: Option<*mut ULONG32>) -> HRESULT {
        unsafe {
            HRESULT(core_bindings::IMetaDataImport2_GetTypeDefProps(metadata, md_type_def, sz_type_def.unwrap_or_default().as_mut_ptr(), cch_type_def.unwrap_or(0), pch_type_def.unwrap_or(&mut 0), pdw_type_def_flags.unwrap_or(0 as _), md_token.unwrap_or(&mut 0)) as u32)
        }
    }
    ...
    pub(crate) fn get_type_ref_props(metadata: &mut IMetaDataImport2,
                                     tr: mdTypeRef,
...
pub mod iro_simple_meta_data_builder {
    ...
    pub(crate) fn set_win_rt_interface(
        ...
        unsafe {
            HRESULT(core_bindings::IRoSimpleMetaDataBuilder_SetWinRtInterface(builder, iid) as u32)
        }
    }
    ...
    pub fn set_enum(
        ...
        unsafe {
            HRESULT(
                core_bindings::IRoSimpleMetaDataBuilder_SetEnum(builder, name, base_type) as u32,
            )
        }
    }
    ...
    pub fn set_struct(
        ...
        unsafe {
            HRESULT(core_bindings::IRoSimpleMetaDataBuilder_SetStruct(
                builder,
                name,
                num_fields,
                field_type_names,
    ...
    pub fn set_delegate(builder: *mut IRoSimpleMetaDataBuilder, iid: GUID) -> HRESULT {
        unsafe { HRESULT(core_bindings::IRoSimpleMetaDataBuilder_SetDelegate(builder, iid) as u32) }
    }
...
pub mod helpers {
    ...
    pub(crate) fn get_type_name(
        ...
        unsafe {
            core_bindings::Helpers_Get_Type_Name(
                metadata.inner() as _,
                md_token,
                name.as_mut_ptr(),
                name.len(),
    ...
    pub(crate) fn cor_sig_uncompress_calling_conv(p_data: PCCOR_SIGNATURE) -> ULONG {
        unsafe { core_bindings::Helpers_CorSigUncompressCallingConv(p_data) }
    }
    ...
    pub(crate) fn cor_sig_uncompress_element_type(
        p_data: PCCOR_SIGNATURE,
    ) -> crate::enums::CorElementType {
        unsafe { core_bindings::Helpers_CorSigUncompressElementType(p_data).into() }
    }

    pub(crate) fn cor_sig_uncompress_token(p_data: PCCOR_SIGNATURE) -> mdToken {
        unsafe { core_bindings::Helpers_CorSigUncompressToken(p_data) }
    }

    pub(crate) fn to_wstring(index: ULONG, text: &mut [u16]) -> usize {
        unsafe { core_bindings::Helpers_to_wstring(index, text.as_mut_ptr()) }
    }
    ...
    pub(crate) fn is_md_instance_initializer_w(flags: DWORD, name_data: *const u16) -> bool {
        unsafe { core_bindings::Helpers_IsMdInstanceInitializerW(flags, name_data) == 1 }
    }

    pub(crate) fn is_pr_special_name(value: DWORD) -> bool {
        unsafe { core_bindings::Helpers_IsPrSpecialName(value) == 1 }
    }

    pub(crate) fn bytes_to_guid(data: *mut u8, guid: *mut GUID) {
        unsafe { core_bindings::Helpers_bytesToGuid(data, guid) }
    }

    pub(crate) fn windows_get_string_raw_buffer(
        mut string: HSTRING,
        length: Option<*mut UINT32>,
    ) -> PCWSTR {
        unsafe {
            core_bindings::Helpers_WindowsGetStringRawBuffer(
                std::mem::transmute(&mut string),
                length.unwrap_or(0 as _),
            )
        }
    }

    pub(crate) fn is_td_interface(value: DWORD) -> bool {
        unsafe { core_bindings::Helpers_IsTdInterface(value) == 1 }
    }

    pub(crate) fn is_td_class(value: DWORD) -> bool {
        unsafe { core_bindings::Helpers_IsTdClass(value) == 1 }
    }

    pub fn is_ev_special_name(value: DWORD) -> bool {
        unsafe { core_bindings::Helpers_IsEvSpecialName(value) == 1 }
    }

    pub(crate) fn generate_id_name(declaration_full_name: &str) -> ([u16], usize) {
        ...
        unsafe {
            core_bindings::Helpers_generate_id_name(
                name_parts_w as _,
                declaration_full_name.as_ptr() as _,
                &mut name_parts_count,
            )
        }
        (*name_parts, name_parts_count)
    }
