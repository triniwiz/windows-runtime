...
impl From<u32> for CorCallingConvention {
    fn from(value: u32) -> Self {
        match value {
            0x0 => CorCallingConvention::ImageCeeCsCallconvDefault,

            0x5 => CorCallingConvention::ImageCeeCsCallconvVararg,
            0x6 => CorCallingConvention::ImageCeeCsCallconvField,
            0x7 => CorCallingConvention::ImageCeeCsCallconvLocalSig,
            0x8 => CorCallingConvention::ImageCeeCsCallconvProperty,
            0x9 => CorCallingConvention::ImageCeeCsCallconvUnmgd,
            0xa => CorCallingConvention::ImageCeeCsCallconvGenericinst,
            // generic method instantiation
            0xb => CorCallingConvention::ImageCeeCsCallconvNativevararg,
            // used ONLY for 64bit vararg PInvoke calls
            0xc => CorCallingConvention::ImageCeeCsCallconvMax, // first invalid calling convention

            // The high bits of the calling convention convey additional info
            0x0f => CorCallingConvention::ImageCeeCsCallconvMask,
            // Calling convention is bottom 4 bits
            0x20 => CorCallingConvention::ImageCeeCsCallconvHasthis,
            // Top bit indicates a 'this' parameter
            0x40 => CorCallingConvention::ImageCeeCsCallconvExplicitthis,
            // This parameter is explicitly in the signature
            0x10 => CorCallingConvention::ImageCeeCsCallconvGeneric, // Generic method sig with explicit number of type arguments (precedes ordinary parameter count)
                                                                     // 0x80 is reserved for internal use
            _ => std::unreachable!()
        }
    }
...
impl From<CorCallingConvention> for u32 {
    fn from(value: CorCallingConvention) -> Self {
        match value {
            CorCallingConvention::ImageCeeCsCallconvDefault => 0x0,

            CorCallingConvention::ImageCeeCsCallconvVararg => 0x5,
            CorCallingConvention::ImageCeeCsCallconvField => 0x6,
            CorCallingConvention::ImageCeeCsCallconvLocalSig => 0x7,
            CorCallingConvention::ImageCeeCsCallconvProperty => 0x8 ,
            CorCallingConvention::ImageCeeCsCallconvUnmgd => 0x9,
            CorCallingConvention::ImageCeeCsCallconvGenericinst => 0xa,
            // generic method instantiation
            CorCallingConvention::ImageCeeCsCallconvNativevararg => 0xb,
            // used ONLY for 64bit vararg PInvoke calls
            CorCallingConvention::ImageCeeCsCallconvMax => 0xc, // first invalid calling convention

            // The high bits of the calling convention convey additional info
            CorCallingConvention::ImageCeeCsCallconvMask => 0x0f,
            // Calling convention is bottom 4 bits
            CorCallingConvention::ImageCeeCsCallconvHasthis => 0x20,
            // Top bit indicates a 'this' parameter
            CorCallingConvention::ImageCeeCsCallconvExplicitthis => 0x40,
            // This parameter is explicitly in the signature
            CorCallingConvention::ImageCeeCsCallconvGeneric => 0x10, // Generic method sig with explicit number of type arguments (precedes ordinary parameter count)
            // 0x80 is reserved for internal use
            _ => std::unreachable!()
        }
    }
...
impl From<core_bindings::CorElementType> for CorElementType {
    fn from(cet: core_bindings::CorElementType) -> Self {
        match cet {
            ...
            core_bindings::CorElementType::ELEMENT_TYPE_BOOLEAN => {
            ...
            core_bindings::CorElementType::ELEMENT_TYPE_VALUETYPE => {
            ...
            core_bindings::CorElementType::ELEMENT_TYPE_GENERICINST => {
                CorElementType::ElementTypeGenericinst
            }
            core_bindings::CorElementType::ELEMENT_TYPE_TYPEDBYREF => {
                CorElementType::ElementTypeTypedbyref
            }
            ...
            core_bindings::CorElementType::ELEMENT_TYPE_SZARRAY => {
                CorElementType::ElementTypeSzarray
            }
            core_bindings::CorElementType::ELEMENT_TYPE_MVAR => CorElementType::ElementTypeMvar,
            core_bindings::CorElementType::ELEMENT_TYPE_CMOD_REQD => {
                CorElementType::ElementTypeCmodReqd
            }
            core_bindings::CorElementType::ELEMENT_TYPE_CMOD_OPT => {
                CorElementType::ElementTypeCmodOpt
            }
            core_bindings::CorElementType::ELEMENT_TYPE_INTERNAL => {
                CorElementType::ElementTypeInternal
            }
            core_bindings::CorElementType::ELEMENT_TYPE_MAX => CorElementType::ElementTypeMax,
            core_bindings::CorElementType::ELEMENT_TYPE_MODIFIER => {
                CorElementType::ElementTypeModifier
            }
            core_bindings::CorElementType::ELEMENT_TYPE_SENTINEL => {
                CorElementType::ElementTypeSentinel
            }
