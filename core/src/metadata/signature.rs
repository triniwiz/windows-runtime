...
impl Signature {
    ...
    pub fn to_string<'a>(metadata: &IMetaDataImport2, signature: PCCOR_SIGNATURE) -> Cow<'a, str> {
        let element_type = helpers::cor_sig_uncompress_element_type(signature);
        return match element_type {
            ...
            CorElementType::ElementTypeValuetype => {
                ...
                let length = helpers::get_type_name(metadata, token, &mut class_name_data);

                let class_name = OsString::from_wide(&class_name_data[..length as usize])
                    .to_string_lossy()
                    .as_ref();
            ...
            CorElementType::ElementTypeVar => {
                ...
                let length = helpers::to_wstring(index, &mut result_data);
                ("Var!".to_owned()
                    + OsString::from_wide(&result_data[..length as usize])
                        .to_string_lossy()
                        .as_ref())
                .into()
            }
