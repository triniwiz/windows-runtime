...
// [windows port] MSVC rejects __attribute__((visibility)). Export the napi_* from the host
// binary via __declspec(dllexport) so napi-rs's Windows runtime symbol lookup
// (libloading GetProcAddress against the current module) can find the statically-linked
// provider. (napi-sys always uses runtime lookup on Windows; there is no link-time variant.)
#ifndef NAPI_EXTERN
#ifdef _WIN32
