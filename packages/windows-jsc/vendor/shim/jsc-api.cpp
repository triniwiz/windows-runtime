...
class JSString {
        ...
        void CopyTo(JSChar* buf, size_t bufsize, size_t* result) const {
            ...
            std::memcpy(buf, chars, size * sizeof(JSChar)); // [windows port] JSChar is 2 bytes; count→bytes
            buf[size] = 0;
            if (result != nullptr) {
                *result = size;
            }
...
class FunctionInfo : public NativeInfo {
        ...
        , _data{data} {
            std::call_once(xyz::functionInfoOnceFlag, []() {
                ...
                // [windows port] make napi functions [[Construct]]-capable so a JS Proxy over them
                // is a constructor (ns_proxy's `new proxy()` needs a constructable target; V8's
                // napi functions are constructable by default, JSC's are not).
                definition.callAsConstructor = FunctionInfo::CallAsConstructor;
...
//            FunctionInfo* info = NativeInfo::Get<FunctionInfo>(function);
        ...
        // JSObjectCallAsConstructorCallback — [windows port] mirrors CallAsFunction with newTarget
        // set. For the ns_proxy pattern the Proxy's own construct trap handles `new`, so this mainly
        // exists to mark the function as a constructor; it also works for a direct `new napiFn()`.
        static JSObjectRef CallAsConstructor(JSContextRef ctx,
                                             JSObjectRef constructor,
                                             size_t argumentCount,
                                             const JSValueRef arguments[],
                                             JSValueRef* exception) {
            FunctionInfo* info = reinterpret_cast<FunctionInfo*>(JSObjectGetPrivate(constructor));
            napi_clear_last_error(info->_env);
            ...
            JSValueRef rv = ToJSValue(result);
            if (rv != nullptr && JSValueIsObject(ctx, rv)) {
                return JSValueToObject(ctx, rv, nullptr);
            }
            return thisObject;
        }
