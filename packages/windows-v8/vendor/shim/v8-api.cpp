...
// across different versions.
        ...
        inline napi_status Unwrap(napi_env env,
            ...
            Reference *reference =
                    static_cast<v8impl::Reference *>(obj->GetAlignedPointerFromInternalField(0, v8::kEmbedderDataTypeTagDefault));
...
napi_status NAPI_CDECL napi_get_property_names(napi_env env,
                                               napi_value object,
                                               napi_value *result) {
    // Implementation from: https://github.com/nodejs/node/blob/57351b628cae6167f03c0417a5e2334da574a743/src/js_native_api_v8.cc
    NAPI_PREAMBLE(env);
    CHECK_ARG(env, result);
...
napi_status NAPI_CDECL napi_get_value_string_latin1(
    ...
    if (!buf) {
        ...
        *result = val.As<v8::String>()->Length();
        ...
        // [windows port] V8 14.7: WriteOneByte → WriteOneByteV2 (offset,length; no return/flags).
        auto str = val.As<v8::String>();
        uint32_t len = static_cast<uint32_t>(str->Length());
        uint32_t to_copy =
                len < static_cast<uint32_t>(bufsize - 1) ? len : static_cast<uint32_t>(bufsize - 1);
        str->WriteOneByteV2(env->isolate, 0, to_copy, reinterpret_cast<uint8_t *>(buf));
        buf[to_copy] = '\0';
        if (result != nullptr) {
            *result = to_copy;
        }
...
napi_status NAPI_CDECL napi_get_value_string_utf8(
    ...
    if (!buf) {
        ...
        *result = val.As<v8::String>()->Utf8LengthV2(env->isolate);
        ...
        // [windows port] V8 14.7: WriteUtf8 → WriteUtf8V2 (null-terminates within capacity;
        // returns bytes copied INCLUDING the null terminator).
        size_t nbytes = val.As<v8::String>()->WriteUtf8V2(
                env->isolate,
                buf,
                bufsize,
...
napi_status NAPI_CDECL napi_get_value_string_utf16(napi_env env,
    ...
    if (!buf) {
        ...
        *result = val.As<v8::String>()->Length();
        ...
        // [windows port] V8 14.7: Write → WriteV2 (offset,length; no return/flags).
        auto str = val.As<v8::String>();
        uint32_t len = static_cast<uint32_t>(str->Length());
        uint32_t to_copy =
                len < static_cast<uint32_t>(bufsize - 1) ? len : static_cast<uint32_t>(bufsize - 1);
        str->WriteV2(env->isolate, 0, to_copy, reinterpret_cast<uint16_t *>(buf));
        buf[to_copy] = '\0';
        if (result != nullptr) {
            *result = to_copy;
        }
...
// [windows port] rusty_v8's extern "C" backing-store bindings (src/binding.cc). Its C++
// ArrayBuffer::NewBackingStore returns a libc++ std::unique_ptr we can't link from this MSVC-STL
// shim, but these C bindings pass the backing store as a raw pointer / an opaque 16-byte shared_ptr
// (two_pointers_t), so we can build a TRUE zero-copy external ArrayBuffer through them.
namespace {
struct RustyTwoPtrs { void *a; void *b; }; // layout-compatible with rusty_v8 two_pointers_t
}
extern "C" {
v8::BackingStore *v8__ArrayBuffer__NewBackingStore__with_data(
        void *data, size_t byte_length, void (*deleter)(void *, size_t, void *),
        void *deleter_data);
RustyTwoPtrs std__shared_ptr__v8__BackingStore__CONVERT__std__unique_ptr(v8::BackingStore *p);
const v8::ArrayBuffer *v8__ArrayBuffer__New__with_backing_store(
        v8::Isolate *isolate, const void *shared_ptr_ref);
...
napi_create_external_arraybuffer(napi_env env,
    ...
    v8::BackingStore *bs = v8__ArrayBuffer__NewBackingStore__with_data(
            external_data, byte_length, deleter, deleter_data);
    RustyTwoPtrs shared = std__shared_ptr__v8__BackingStore__CONVERT__std__unique_ptr(bs);
    const v8::ArrayBuffer *raw = v8__ArrayBuffer__New__with_backing_store(isolate, &shared);
    std__shared_ptr__v8__BackingStore__reset(&shared); // ArrayBuffer holds its own ref now
    v8::Local<v8::ArrayBuffer> buffer = *reinterpret_cast<v8::Local<v8::ArrayBuffer> *>(&raw);
...
// [BABYLON-NATIVE-ADDITION]
napi_status napi_run_script_source(napi_env env,
                                   napi_value script,
    ...
    const auto v8_script_string = v8::Local<v8::String>::Cast(
            v8impl::V8LocalValueFromJsValue(script));
    const auto source_with_comment = v8::String::Concat(env->isolate, v8_script_string,
                                                        OneByteString(env->isolate,
                                                                      source_url_comment_str.c_str(),
                                                                      source_url_comment_str.size()));
        ...
        static v8::Intercepted IndexedSetter(uint32_t index, v8::Local<v8::Value> value,
            ...
            if (hostObject->is_array_) {
                ...
                napi_status status = napi_call_function(hostObject->env_,
                                                        ...
                                                        argv, &result);
                if (status == napi_ok) {
                    // [windows port] V8 14.7 setter interceptors return Intercepted (ReturnValue<void>).
                    return v8::Intercepted::kYes;
                } else {
                    napi_value ex;
                    napi_get_and_clear_last_exception(hostObject->env_, &ex);
...
napi_create_host_object(napi_env env, napi_value value, napi_finalize finalize, void *data,
    ...
    // Convert napi_value to v8::Local<v8::Value>
    v8::Local<v8::Value> v8Value = v8impl::V8LocalValueFromJsValue(value);

    // Check if the template is already cached
    if (env->host_object_template.IsEmpty()) {
        // Create an ObjectTemplate for the host object
        v8::Local<v8::ObjectTemplate> hostObjectTemplate = v8::ObjectTemplate::New(isolate);
        hostObjectTemplate->SetInternalFieldCount(4); // To store the NapiHostObject instance

        // Define the property handler configuration
        v8::NamedPropertyHandlerConfiguration handlerConfiguration(
                v8impl::NapiHostObject::Getter,
                v8impl::NapiHostObject::Setter,
                v8impl::NapiHostObject::Query,
                v8impl::NapiHostObject::Deleter,
                v8impl::NapiHostObject::Enumerator,
                v8::Local<v8::Value>(), // Data
                v8::PropertyHandlerFlags::kNonMasking
        );

        // Set the property handler configuration on the template
        hostObjectTemplate->SetHandler(handlerConfiguration);

        // Define the indexed property handler configuration
        v8::IndexedPropertyHandlerConfiguration indexedHandlerConfiguration(
        ...
        hostObjectTemplate->SetHandler(indexedHandlerConfiguration);


        // Cache the template
        env->host_object_template.Reset(isolate, hostObjectTemplate);
    }


    // Create an instance of the host object
    v8::Local<v8::ObjectTemplate> hostObjectTemplate = env->host_object_template.Get(isolate);
    v8::Local<v8::Object> hostObject = hostObjectTemplate->NewInstance(context).ToLocalChecked();

    v8::Local<v8::Value> v8Getter;
    v8::Local<v8::Value> v8Setter;
    if (is_array) {
        v8Getter = v8impl::V8LocalValueFromJsValue(getter);
        v8Setter = v8impl::V8LocalValueFromJsValue(setter);

    }

    // Store the NapiHostObject instance in the internal field
    ...
    *result = v8impl::JsValueFromV8LocalValue(hostObject);
    ...
    // Set the finalizer if provided
    if (finalize != nullptr) {
        v8impl::Reference::New(env, hostObject, 0, v8impl::Ownership::kRuntime,
                               [](napi_env env, void *d1, void *d2) {
                                   auto hostInfo = reinterpret_cast<v8impl::NapiHostObject *>(d1);
                                   if (hostInfo->finalize_) {
                                       hostInfo->finalize_(env, hostInfo->data_, nullptr);
                                   }
                                   delete hostInfo;
                               }, hostInfo, nullptr);
    }
