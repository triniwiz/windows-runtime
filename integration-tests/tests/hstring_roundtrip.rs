...
use windows::Data::Json::{IJsonValue, IJsonValueStatics, JsonValue};
...
fn hstring_libffi_roundtrip() {
    unsafe {
        ...
        println!(
            "[DIAG TEST] statics_ptr={:p} vtable_ptr_ptr={:p} vtable_ptr={:p}",
            statics_ptr, vtable_ptr_ptr, vtable_ptr
        );
        write_log("[DIAG TEST] got statics and vtable");

        // Typed direct vtable call for baseline frame comparison
        type CreateStringValueFn = unsafe extern "system" fn(
            this: *mut c_void,
            value: HSTRING,
        ...
        let typed_hr: i32 = func_typed(
            statics_ptr as *mut c_void,
            typed_h.clone(),
            &mut typed_result,
        );
        assert_eq!(
            typed_hr, 0,
            "typed CreateStringValue failed hr={:#x}",
            typed_hr
        );
        assert!(
            !typed_result.is_null(),
            "typed CreateStringValue returned null"
        );
        ...
        let typed_ijv: IJsonValue = unknown_typed
            .cast::<IJsonValue>()
            .expect("cast to IJsonValue failed");
        ...
        let libffi_s = runtime::diag_libffi_create_string_value_via_runtime("hello-roundtrip")
            .expect("runtime libffi wrapper failed");
