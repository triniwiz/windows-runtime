...
fn libffi_create_values() {
    unsafe {
        ...
        eprintln!("[LIBFFI] statics_ptr={:p} vtable_ptr={:p}", statics_ptr, vtable_ptr);
        for i in 0..12isize {
        ...
        // CreateBooleanValue(this, bool, out **obj) -> HRESULT
        {
            ...
            type CreateBoolFn = unsafe extern "system" fn(this: *mut c_void, value: u8, result: *mut *mut c_void) -> i32;
            ...
            eprintln!("[LIBFFI] direct CreateBooleanValue hr={:#x} result={:p}", direct_hr, direct_result);
            assert_eq!(direct_hr, 0, "direct CreateBooleanValue failed hr={:#x}", direct_hr);
            ...
            frame_i32.extend_from_slice(&((&mut result as *mut _ as usize).to_le_bytes())[0..ptr_size]);
            let mut hex = String::new();
            for (i, b) in frame_i32.iter().enumerate() {
                hex.push_str(&format!("{:02x}", b));
                if i % 4 == 3 { hex.push(' '); }
            }
            eprintln!("[LIBFFI] libffi contiguous frame (bool as u8) total_size={} bytes={}", frame_i32.len(), hex);

            let hr: i32 = cif.call(CodePtr::from_ptr(func_ptr), &[
                Arg::new(&this_ptr_usize),
                Arg::new(&arg_bool_u32),
                Arg::new(&mut result_usize),
            ]);
            if result_usize != 0 { result = result_usize as *mut c_void; }
            eprintln!("[LIBFFI] CreateBooleanValue returned hr={:#x} result={:p}", hr, result);
        ...
        // CreateNumberValue(this, f64, out **obj) -> HRESULT
        {
            ...
            if result_usize != 0 { result = result_usize as *mut c_void; }
            eprintln!("[LIBFFI] CreateNumberValue returned hr={:#x} result={:p}", hr, result);
...
fn libffi_create_string_value() {
    unsafe {
        ...
        type CreateStringFn = unsafe extern "system" fn(this: *mut c_void, value: windows::core::HSTRING, result: *mut *mut c_void) -> i32;
        ...
        eprintln!("[LIBFFI] typed CreateStringValue hr={:#x} result={:p} typed_h_raw={:#x}", typed_hr, typed_result, typed_h_raw);
        ...
        if result_usize != 0 { result = result_usize as *mut c_void; }
        eprintln!("[LIBFFI] libffi CreateStringValue hr={:#x} result={:p} libffi_h_raw={:#x}", hr, result, h_raw);
