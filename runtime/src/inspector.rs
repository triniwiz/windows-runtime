...
pub(crate) fn handle_register_domain_dispatcher(
    ...
    if args.length() < 2 {
        throw_js_error(
            scope,
            "__registerDomainDispatcher(domain, dispatcher) expects 2 arguments",
        );
        return;
    }
    ...
    if !dispatcher_val.is_object() && !dispatcher_val.is_function() {
        throw_js_error(
            scope,
            "dispatcher must be an object or constructor function",
        );
        return;
    }
    ...
    if dispatcher_val.is_function() {
        if let Ok(_ctor) = v8::Local::<v8::Function>::try_from(dispatcher_val) {
            // Build a tiny function expression that constructs the ctor and
            // returns the instance (or null on error).
            if let Some(src) = v8::String::new(
                scope,
                "(function(ctor){ try { return new ctor(); } catch (e) { return null; } })",
            ) {
...
pub(crate) fn handle_inspector_send_event(
    ...
    let _json = sv.to_rust_string_lossy(scope);
...
pub fn try_dispatch_inspector_message_to_js(msg: &str) -> bool {
    ...
    // when unnecessary.
    let method = match serde_json::from_str::<JsonValue>(msg).ok().and_then(|v| {
        v.get("method")
            .and_then(|m| m.as_str())
            .map(|s| s.to_string())
    }) {
    ...
    let Some(dg) = dispatcher_global else {
        return false;
    };
    ...
    let Some(msg_str) = v8::String::new(tc, msg) else {
        return false;
    };
    let Some(msg_val) = v8::json::parse(tc, msg_str) else {
        return false;
    };
    ...
    let params_val = msg_obj
        .get(tc, params_key.into())
        .unwrap_or_else(|| v8::undefined(tc).into());

    let recv = v8::Local::new(tc, &dg);
    let obj = if let Ok(o) = v8::Local::<v8::Object>::try_from(recv) {
        o
    } else {
        return false;
    };
    ...
    if method_val.is_none() || method_val.unwrap().is_undefined() {
        return false;
    }
...
pub(crate) fn maybe_attach_devtools(scope: &mut v8::ContextScope<v8::HandleScope<v8::Context>>) {
    ...
    let forwarder =
        Some(Arc::new(|s: &str| crate::debug_output(s)) as Arc<dyn Fn(&str) + Send + Sync>);
    let dispatcher = Some(
        Arc::new(|msg: &str| try_dispatch_inspector_message_to_js(msg))
    ...
    match runtime_devtools::DevtoolsServer::attach(
        &config,
        isolate,
        &global_context,
        forwarder,
