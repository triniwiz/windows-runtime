...
#include "Inspector/src/inspector/v8-log-agent-impl.h"
...
namespace org {
	namespace nativescript {
		void Console::Init(Local<Context> context)
		{
			...
			if (!global->DefineOwnProperty(context, ToV8String(isolate, "console"), console, readonlyFlags).FromMaybe(false)) {
				// TODO assert
			};
		}
        ...
        void Console::LogCallback(const FunctionCallbackInfo <Value>& args) {
            ...
            std::string verbosityLevel = ToString(isolate, data);
            std::string verbosityLevelUpper = verbosityLevel;
            std::transform(verbosityLevelUpper.begin(), verbosityLevelUpper.end(), verbosityLevelUpper.begin(),
                ::toupper);
            ...
            if (verbosityLevel == "trace") {
                std::string stacktrace = GetStackTrace(isolate);
        ...
        void Console::DirCallback(const FunctionCallbackInfo <Value>& args) {
            ...
            std::stringstream ss;
            std::string scriptUrl = GetCurrentScriptUrl(isolate);
            ss << scriptUrl << ":";
            ...
            if (argsLen > 0) {
                if (!args[0]->IsObject()) {
                    ...
                    for (uint32_t i = 0; i < propertiesLength; i++) {
                        ...
                        ss << ToString(isolate, propertyName->ToString(context).ToLocalChecked()) << ": ";
                        ...
                        if (propIsFunction) {
                            ...
                            std::string jsonStringifiedArray = ToString(isolate, stringResult);
                            ...
                            std::string jsonStringifiedObject = ToString(isolate, objString);
                            ...
                            ss << "\""
                                << ToString(isolate, propertyValue->ToDetailString(context).ToLocalChecked())
                                << "\"";
                        }
        ...
        void Console::TimeCallback(const FunctionCallbackInfo <Value>& args) {
            ...
            if (args.Length() > 0 && args[0]->ToString(context).ToLocal(&labelString)) {
                label = ToString(isolate, labelString);
            }
        ...
        void Console::AttachLogFunction(Local <Context> context, Local <Object> console, const std::string name,
            v8::FunctionCallback callback) {
            Isolate* isolate = context->GetIsolate();

            Local <v8::Function> func;
            if (!Function::New(context, callback, ToV8String(isolate, name), 0,
                ConstructorBehavior::kThrow).ToLocal(&func)) {
                // tns::Assert(false, isolate);
            }

            Local <v8::String> logFuncName = ToV8String(isolate, name);
            ...
            if (val->IsFunction()) {
                ...
                for (uint32_t i = 0; i < arrayLength; i++) {
                    ...
                    if (propertyValue->StrictEquals(cachedSelf)) {
                        argString = v8::String::Concat(isolate, argString, ToV8String(isolate, "[Circular]"));
                        continue;
                    }
                    ...
                    if (i != arrayLength - 1) {
                        argString = v8::String::Concat(isolate, argString, ToV8String(isolate, ", "));
                    }
                }
        ...
        const Local <v8::String> Console::TransformJSObject(Local <Object> object) {
            ...
            bool hasCustomToStringImplementation =
                ToString(isolate, objToString).find("[object Object]") == std::string::npos;

            if (hasCustomToStringImplementation) {
                resultString = objToString;
            }
            else {
                resultString = JsonStringifyObject(context, object);
            }
