...
namespace org {
	namespace nativescript {
        ...
        void NativeScriptException::OnUncaughtError(Local<Message> message, Local<Value> error) {
            ...
            Local<Value> handler;
            bool value = false; //Runtime::GetAppConfigValue<bool>("discardUncaughtJsExceptions");
            bool isDiscarded = value;

            std::string cbName = isDiscarded ? "__onDiscardedError" : "__onUncaughtError";
            bool success = global->Get(context, ToV8String(isolate, cbName)).ToLocal(&handler);

            std::string stackTrace = GetErrorStackTrace(isolate, message->GetStackTrace());
            if (success && handler->IsFunction()) {
                if (error->IsObject()) {
                   Assert(error.As<Object>()->Set(context, ToV8String(isolate, "stackTrace"), ToV8String(isolate, stackTrace)).FromMaybe(false), isolate);
                }
            ...
            if (!isDiscarded) {
                ...
                std::string messageString = ToString(isolate, messageV8String);
                std::string name = std::string("NativeScript encountered a fatal error: " + messageString + "\n at \n" + stackTrace);
            ...
            if (this->javascriptException_ != nullptr) {
                ...
                if (errObj->IsObject()) {
                    if (!this->fullMessage_.empty()) {
                            Assert(errObj.As<Object>()->Set(context, ToV8String(isolate, "fullMessage"), ToV8String(isolate, this->fullMessage_)).FromMaybe(false), isolate);
                    }
                    else if (!this->message_.empty()) {
                            Assert(errObj.As<Object>()->Set(context, ToV8String(isolate, "fullMessage"), ToV8String(isolate, this->message_)).FromMaybe(false), isolate);
                    }
                }
            }
            else if (!this->fullMessage_.empty()) {
                errObj = Exception::Error(ToV8String(isolate, this->fullMessage_));
            }
            else if (!this->message_.empty()) {
                errObj = Exception::Error(ToV8String(isolate, this->message_));
            }
            else {
                errObj = Exception::Error(ToV8String(isolate, "No javascript exception or message provided."));
            }
        ...
        std::string NativeScriptException::GetErrorMessage(Isolate* isolate, Local<Value>& error, const std::string& prependMessage) {
            ...
            auto v8FullMessage = ToV8String(isolate, "fullMessage");
            if (error->IsObject() && error.As<Object>()->Has(context, v8FullMessage).ToChecked()) {
                ...
                if (!errMsgVal.IsEmpty()) {
                    errMessage = ToString(isolate, errMsgVal.As<v8::String>());
                }
                else {
                    errMessage = "";
                }
        ...
        std::string NativeScriptException::GetErrorStackTrace(Isolate* isolate, const Local<StackTrace>& stackTrace) {
            ...
            for (int i = 0; i < frameCount; i++) {
                ...
                std::string funcName = ToString(isolate, frame->GetFunctionName());
                std::string srcName = ToString(isolate, frame->GetScriptName());
        ...
        std::string NativeScriptException::GetFullMessage(Isolate* isolate, const TryCatch& tc, const std::string& jsExceptionMessage) {
            ...
            if (!scriptResName.IsEmpty() && scriptResName->IsString()) {
                ss << std::endl << "File: (" << ToString(isolate, scriptResName.As<v8::String>());
            }
            else {
